...
export default function AnomalyModal({
  ...
  return createPortal(
    <div
      className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-[var(--overlay-scrim)] backdrop-blur-sm"
      onClick={onClose}
      role="dialog"
      aria-modal="true"
    >
      ...
      >
        <div className="flex items-start justify-between gap-4 px-5 py-4 border-b border-[var(--border-soft)] shrink-0">
          ...
          <button
            type="button"
            onClick={onClose}
            className="shrink-0 rounded-lg p-1.5 text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-[var(--surface-glass-06)] transition"
            aria-label="Close"
          >
            <X size={16} />
          </button>
